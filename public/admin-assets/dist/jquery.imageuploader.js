...
(function ($) {
    $.fn.uploader = function (options, testMode) {
        return this.each(function (index) {
            ...
            var dom = {
                uploaderBox: $(this),
                submitButton: $('<button id="uploading" class="btn js-uploader__submit-button uploader__submit-button uploader__hide">' +
                    options.submitButtonCopy + '<i class="js-uploader__icon icon-upload uploader__icon"></i></button>'),
                ...
                furtherInstructions: $('<p id="extra" class="button--big-bottom js-uploader__further-instructions uploader__further-instructions uploader__hide">' + options.furtherInstructionsCopy + '</p>')
            };
            ...
            function addItem (file) {
                $('#uploading').show();
                $('#uploading').text('Upload Selected Files');
                ...
                var thumbnail = $('<img class="thumbnail"><i class="icon-spinner12 icon-spin uploader__icon--spinner"></i>');
                var removeLink = $('<span class="uploader__file-list__button"><button class="uploader__icon-button js-upload-remove-button icon-times" data-index="' + id + '"></button></span>');
                ...
                if (window.FileReader && file.type.indexOf('image') !== -1) {
                    ...
                    thumbnail = $('<i class="icon-file-o uploader__icon">');
                }
                ...
                if (state.fileBatch.length !== 0) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            for (var i = 0; i < state.fileBatch.length; i++) {
                                dom.fileList.find('li[data-index="' +state.fileBatch[i].id + '"]').remove();
                    
                            }
                            state.fileBatch=[];
                            // options = {'ajaxUrl':"{{ route('admin.settings.addImage') }}",'reload':false};
                            $('.js-uploader__box').uploader(options);
                            // $('#uploadImagesModal').removeChild($('#uploadImagesModal').lastChild);
